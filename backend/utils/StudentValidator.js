...
class StudentValidator {
  static validateForCreate(studentData) {
    ...
    // Validar blood_type
    const validBloodTypes = ['A+', 'A-', 'B+', 'B-', 'AB+', 'AB-', 'O+', 'O-', 'No informado'];
    if (studentData.blood_type && !validBloodTypes.includes(studentData.blood_type)) {
      errors.push(`Grupo sanguíneo inválido. Valores válidos: ${validBloodTypes.join(', ')}`);
    }

    // Validar guardians array
    if (studentData.guardians && Array.isArray(studentData.guardians)) {
      studentData.guardians.forEach((g, index) => {
        if (!g.first_name || !g.paternal_surname || !g.dni) {
          errors.push(`Guardián #${index + 1}: Nombre, Apellido y DNI son requeridos`);
        }
      });
    }
  ...
  static validateForUpdate(studentData, studentId) {
    ...
    // Validar guardians array for updates
    if (studentData.guardians && Array.isArray(studentData.guardians)) {
      studentData.guardians.forEach((g, index) => {
        if (!g.first_name || !g.paternal_surname || !g.dni) {
          errors.push(`Guardián #${index + 1}: Nombre, Apellido y DNI son requeridos`);
